...
def rank_plot(q_reg, userdata_resampled_int, cif_rank_pearson_list, cif_dict, png_path):
    ...
    for i in range(0, 5):
        ...
        for key in cif_dict:
            if key == file:
                cifdata_resampled.append(cif_dict[key]['intensity_resampled'])
    ...
    for i in range(1, 6):
        y_min, y_max = np.amin(cifdata_resampled[i - 1]), np.amax(cifdata_resampled[i - 1])
        y_range = y_max - y_min
        axs[i].plot(q_reg, cifdata_resampled[i - 1], lw=0.5, c=colors[i])
